...
import { SeismicLog, LevelConfig, IntensityLevel } from '../types';
...
export const HistoryList: React.FC<Props> = ({ logs, onDelete }) => {
  ...
  const filteredLogs = useMemo(() => {
    ...
    return logs.filter(log => log.intensity === filterLevel);
  }, [logs, filterLevel]);
    ...
    <div className="w-full max-w-md mx-auto p-4 pb-20 pt-8">
      <div className="flex items-center justify-between mb-6 ml-2 pr-2">
        ...
        <select 
          ...
          className="bg-white/50 border border-slate-200 rounded-xl px-3 py-1.5 text-[10px] font-black text-slate-600 focus:outline-none focus:ring-2 focus:ring-blue-500/20 transition-all appearance-none cursor-pointer pr-8 relative"
          style={{
        ...
        >
          ...
          {[6, 5, 4, 3, 2, 1].map(level => (
            <option key={level} value={level}>L{level} {LevelConfig[level as IntensityLevel].alertName}</option>
          ))}
        </select>
      </div>
